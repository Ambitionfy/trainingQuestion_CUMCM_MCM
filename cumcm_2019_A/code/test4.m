...
for t0 = t0s% 初始时高压油泵的延时 
    ...
    for n = 1:N % 离散计算 
        ...
        inQ(n) = inQual; % 记录值 % 出油总质量 
        outQual = outFuel(time, density); 
        outQ(n) = outQual;% 记录值 % 更新密度 
    ...
    Err3 = sum(abs(Err1))/N; % 波动误差 %% 数据整理 %绘制压力曲线图 
    figure 
    ...
    ylabel('压强差 P(Mpa)') 
    set(gcf,'units','centimeters') % 标准单位：厘米 
    set(gcf,'InnerPosition',[16 5 16 8]) % 在屏幕的位置，图窗的长宽 %绘制进油质量与出油质量图 
    figure 
    subplot(2,1,1) 
...
%% 计算进油工作周期
    ...
    for Tt1 = 27:30 % 搜索法，进油工作周期 
        ...
        for n = 1:N % 离散计算 
            time = n * dt; % 计算实际时间 % 进油 
            inQual = inFuel(time, density, t0, Tt1*dt);%In Fuel 是进油计算函数 % 出油 
            outQual = outFuel(time, density);%out Fuel 是出油计算函数 % 是否在增压 % 更新密度 
