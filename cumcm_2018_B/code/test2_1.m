...
target = 300;%最大目标数量
[num,order1] = getAssign(tprocess1,tprocess2);%获取较合适的策略
for j = 1 : length(order1)
   ...
   %第一轮
   for i = 1 : num
       pos = cnc_pos(order1(j,i));%找出当前该去的cnc位置
       if pos == rgv_pos
          result(i,1) = i;
          result(i,2) = order1(j,i);
          result(i,3) = time; %不需移动，直接上料
          time = time + tud(order1(j,i));%更新时间
          cnc_endtime(order1(j,i)) = time + tprocess1;%记录该cnc完成时间
          cnc_now(order1(j,i)) = i;%该cnc当前在加工第i个物料
       else
           ...
           time = time + tud(order1(j,i));%更新上料时间
   ...
   order1(j,num+1) = result(target-10,8);
end
[xtemp,~]=find(order1==min(order1(:,num+1))); % 寻找最优的那（些）组参数
answer = order1(xtemp,:)
...
function [num,order] = getAssign(tprocess1,tprocess2)
%获取两个不同加工时间差最小的排列组合
%i个进行一类加工 -- 8-i个进行二类加工
xmin = 1e5;
order = [];
for i = 1 : 7
   temp = abs(i/tprocess1 - (8-i)/tprocess2);
   if temp < xmin
      xmin = temp;
      num = i;
   end
end
xtemp = nchoosek(1:8,num);
%找出所有排列方式
for i = 1 : length(xtemp)
    %对所有排列方式进行全排列
   order = [order;perms(xtemp(i,:))];
end
